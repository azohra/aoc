...
export const solvePart1 = async (input: string): Promise<number> => {
  const [map, movements] = parseInput(input);
  return gpsSum(await solve(structuredClone(map), movements, 1));
};

export const solvePart2 = async (input: string): Promise<number> => {
  const [map, movements] = parseInput(input);
...
if (import.meta.main) {
  console.log("Answer (P1):", await solvePart1(input));
  console.log("Answer (P2):", await solvePart2(input));
}
